#' The Bootstrap P-Value Function
#'
#' @param x sample from the population, used for one-sample t.test for p-value
#' @param conf.level confidence level, 1 - alpha, for the confidence interval for mu
#' @param iter used for iterations of bootstrap intervals, increase for more accuracy
#' @param mu0 NULL value to test sample mean against for rejecting or not rejecting the NULL hypothesis H_0
#' @param test test type, "two" for two-tailed test, "upper" for right-tailed test, or "lower" for left-tailed test
#'
#' @return named list with p-value, t_calc, length of vector, data vector passed, test type passed, and confidence interval
#' @export
#' @importFrom graphics title
#'
#' @examples
#' bootpval(x = rnorm(30, 25, 5), mu0 = 24)
bootpval <- function(x, conf.level = 0.95, iter = 3000, mu0 = 0, test = "two") {
  ...
  tstat <- function(z) { # The value of t when the NULL is assumed true (zbar-mu0)/(s/sqrt(n))
